...
for i=1:length(P)
    ...
    elseif P(i,3) == 2 %trial type 2 = mixed gamble with ambiguous risky loss vs sure 0
        U1 = 0.5*P(i,4)^x(3) - 0.5*x(2)*8.15^x(3); %8.15 = mean loss value for conditions 1 and 3
        U2 = 0;
    elseif P(i,3) == 3 %trial type 3 = mixed gamble with ambiguous risky gain vs sure 0
        U1 = 0.5*14.15^x(3) - 0.5*x(2)*(abs(P(i,5)))^x(3); %14.15 = mean gain value for conditions 1 and 2
        U2 = 0;
    ...
    elseif P(i,3) == 5 %trial type 5 = gain-only gamble with ambiguous risky gain vs sure gain
        U1 = 0.5*15^x(3); %15 = mean risky gain value for conditions 4 and 6
        U2 = P(i,6)^x(3);
    elseif P(i,3) == 6 %trial type 6 = gain-only gamble vs sure ambiguous gain
        U1 = 0.5*P(i,4)^x(3);
        U2 = 5.50^x(3); %5.50 = mean sure gain value for conditions 4 and 5
    elseif P(i,3) == 7 %trial type 7 = sure ambiguous gain vs sure known gain
        U1 = 7.00^x(3); %7.00 = mean gain value for condition 7 
        U2 = P(i,6)^x(3);
    elseif P(i,3) == 8 %trial type 8 = sure ambiguous loss vs sure known loss
        U1 = -x(2)*7.00^x(3); %7.00 = mean loss value for condition 8
