...
% x(4) = ambiguity parameter (>0)
      
function f = LL_function_Model2(x,P)
...
x(4) = 3/(1+exp(-x(4))); % ambiguity parameter [0 3]
...
for i=1:length(P)
    ...
    elseif P(i,3) == 5 %trial type 5 = gain-only gamble with ambiguous risky gain vs sure gain
        U1 = 0.5*x(4)*75^x(3); %75 = mean gain value for conditions 4 and 6
        U2 = P(i,6)^x(3);
    elseif P(i,3) == 6 %trial type 6 = gain-only gamble vs sure ambiguous gain
        U1 = 0.5*P(i,4)^x(3);
        U2 = x(4)*28^x(3); %28 = mean loss value for conditions 4 and 5
