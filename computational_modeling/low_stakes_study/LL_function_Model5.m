...
function f = LL_function_Model5(x,P)

%transform parameters to make sure they are constrained between values that
%make sense, for ex
x(1) = 1/(1+exp(-x(1))); % mu [0 1]
x(2) = 10/(1+exp(-x(2))); % lambda [0 10]
x(3) = 3/(1+exp(-x(3)));  % rho [0 3]
x(4) = 3/(1+exp(-x(4)));  % ambiguity preference risky gains C3-5 [0 3]
x(5) = 3/(1+exp(-x(5)));  % ambiguity aversion risky loss C2 [0 3]
x(6) = 3/(1+exp(-x(6)));  % ambiguity preference sure gain C6-7 [0 3]
x(7) = 3/(1+exp(-x(7)));  % ambiguity aversion sure loss C8 [0 3]
...
for i=1:length(P)
    ...
    elseif P(i,3) == 2 %trial type 2 = mixed gamble with ambiguous risky loss vs sure 0
        U1 = 0.5*P(i,4)^x(3) - 0.5*x(2)*x(5)*41^x(3); %41 = mean loss value for conditions 1 and 3
        U2 = 0;
    elseif P(i,3) == 3 %trial type 3 = mixed gamble with ambiguous risky gain vs sure 0
        U1 = 0.5*x(4)*71^x(3) - 0.5*x(2)*(abs(P(i,5)))^x(3); %71 = mean gain value for conditions 1 and 2
        U2 = 0;
    ...
    elseif P(i,3) == 5 %trial type 5 = gain-only gamble with ambiguous risky gain vs sure gain
        U1 = 0.5*x(4)*75^x(3); %75 = mean risky gain value for conditions 4 and 6
        U2 = P(i,6)^x(3);
    elseif P(i,3) == 6 %trial type 6 = gain-only gamble vs sure ambiguous gain
        U1 = 0.5*P(i,4)^x(3);
        U2 = x(6)*28^x(3); %28 = mean sure gain value for conditions 4 and 5
    elseif P(i,3) == 7 %trial type 7 = sure ambiguous gain vs sure known gain
        U1 = x(6)*35^x(3); %35 = mean gain value for condition 7 
        U2 = P(i,6)^x(3);
    elseif P(i,3) == 8 %trial type 8 = sure ambiguous loss vs sure known loss
        U1 = -x(2)*x(7)*35^x(3); %35 = mean loss value for condition 8
